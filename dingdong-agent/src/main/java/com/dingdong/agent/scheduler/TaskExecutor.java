package com.dingdong.agent.scheduler;
...
import com.dingdong.core.scheduler.ScheduleStore.ScheduleEntry;
...
public class TaskExecutor {
    ...
    private void executeAiGenerate(ScheduleEntry entry) {
        ...
        // 使用自定义配置调用 Agent
        com.dingdong.agent.agent.AgentConfig config = com.dingdong.agent.agent.AgentConfig.builder()
        ...
        agent.chat(isPrivate ? targetId : 0, isPrivate ? 0 : targetId, finalPrompt, config, null)
                .thenAccept(reply -> {
                    if (reply != null && !reply.isBlank()) {
                        // 构建消息链
                        com.dingdong.core.message.MessageChain msg;
                        Long createdBy = entry.getCreatedBy();
                        
                        if (!isPrivate && createdBy != null && createdBy > 0) {
                            msg = new com.dingdong.core.message.MessageChain()
                                    .at(createdBy)
                                    .text(" " + reply);
                        } else {
                            msg = com.dingdong.core.message.MessageChain.ofText(reply);
                        }
